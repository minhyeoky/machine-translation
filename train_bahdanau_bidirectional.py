...
from src.model.seq2seq_bahdanau_bidirectional import Encoder, Decoder
...
vocab_size_ko = data_loader.tar_vocab_size
vocab_size_en = data_loader.ori_vocab_size
...
def train_step(ori_train, tar_train):
    ...
    with tf.GradientTape() as tape:
        ...
        for time_step in range(1, len_tar):
            ...
            time_loss = keras.losses.sparse_categorical_crossentropy(
                y_true=labels, y_pred=logits, from_logits=True
            )
            time_loss *= tf.cast(
                tf.reshape(mask[:, time_step], (-1,)), dtype=tf.float32
            )
            loss += time_loss
            # Teacher forcing - at training time, only labels are fed
            input_decoder = tf.expand_dims(
                tar_train[:, time_step], 1
            )  # [batch_size, 1]
...
def inference(inference_data):
    ...
    input_decoder = tf.expand_dims(
        [tokenizer_ori.word_index["<start>"]] * inference_size, axis=1
    )
