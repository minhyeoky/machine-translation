...
class DataLoader(DataLoaderBase):
    def __init__(
        ...
        self.tokenizer = NamedTuple(
            "Tokenizer", [("ori", tokenizer), ("tar", tokenizer)]
        )

        self.tokenizer.ori = tokenizer(
            num_words=num_words, filters="", lower=True, split=" ", oov_token=OOV_TOKEN
        )

        self.tokenizer.tar = tokenizer(
            num_words=num_words, filters="", lower=True, split=" ", oov_token=OOV_TOKEN
        )
    ...
    def _tokenize(self, texts, tokenizer, fit):
        ...
        sequences = tf.keras.preprocessing.sequence.pad_sequences(
            sequences, maxlen=self.maxlen, padding="post", truncating="post", value=0
        )  # padding value
        return sequences
